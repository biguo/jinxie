...
class AuthController extends Controller
{
    ...
    {
        $credentials = $request->only(['username', 'name', 'password']);

        $validator = Validator::make($credentials, [
            'username' => 'required', 'name' => 'required','password' => 'required',
        ]);
        $errors = $validator->errors()->getMessages();
        $count = Administrator::where([['username','=',$credentials['username']]])->count();
        if($count > 0){
            $errors['username'] = ['The username field has used.'];
        }
